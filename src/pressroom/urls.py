...
urlpatterns += patterns('',
    url(r'^(?P<year>\d{4})/(?P<month>[a-z]{3})/(?P<day>\w{1,2})/(?P<language>[\-\d\w]+)/(?P<slug>[\-\d\w]+)/$', ArticleDetailView.as_view(slug_field='slug', queryset= Article.objects.get_published()), name='pr-article-detail'),
